...
for (i in G ) {
	...
	lines(xNew,yNew, col="red")
					...
					######## IUCNhab,Order,DistC,Family,Genus ########
					
setwd("C:/Users/Konstantina/Desktop/Distance/Comparison/merged_tables")

Com<- read.table("CSnE_Disp.txt", h=T);Com<- Com[c(1,2,5,6,7,8)]
Com2<- read.table("CSnE_info.txt", h=T);Com2<- Com2[c(1,11,12,19,23,25)]
...
#use of IUCNhab, Order, DistC, Family, Genus
...
for (i in unique(Com3$Genus)){##change here
	
	a<- subset(Com3, Genus==i)##here;#subset
